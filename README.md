...
a parked domain that refused to stay parked. it's alive at **[uxuiai.org](https://uxuiai.org)**. (not to be confused with [ux-ui.ai](https://www.ux-ui.ai), the consultancy — that's a separate repo.)
...
this lives on **AWS Amplify** at [uxuiai.org](https://uxuiai.org). amplify watches the main branch and deploys on push. no build config needed beyond what's already in `package.json` — vite outputs to `dist/` and amplify finds it.
